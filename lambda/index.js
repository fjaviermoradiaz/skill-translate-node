...
async function translate(textToTranslate, sourceLang, targetLang) {
    ...
    return new Promise(async(resolve, reject) => {
        const result = await translate.translateText(params, function (err, data) {
            if (err) {
                reject(err)
            }
            if (data) {
...
const TranslateIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        if(handlerInput.requestEnvelope.request.intent.slots.toTranslate) {
            ...
            translateOutput = await translate(textToTranslate,'es','en')
        }
        console.log("textTranslated: " + translateOutput.TranslatedText);
        return handlerInput.responseBuilder
            .speak('<lang xml:lang="en-US">'+translateOutput.TranslatedText+'</lang>')
            .getResponse();
    }
};
